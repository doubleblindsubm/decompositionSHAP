...
class DependentKernelExplainer(KernelExplainer):
    ...
    interventional and dependent effect. Uses Kernel SHAP to compute conditional SHAP values and SamplingExplainer
    compute interventional effects.
    """
    def __init__(self, model, data, dist, link=None, **kwargs):
        """
        ...
        :param dist: Distribution object from distributions.py
            The data distribution from which to sample by dist.sample(...)
    ...
    def shap_values(self, X, **kwargs):
        ...
        :param neval_kernel: "auto" or int
            Number of model evaluations for permutation to compute conditional SHAP values using Kernel SHAP.
        ...
        :return:
            For models with a single output this returns a matrix of SHAP values and interventional
    ...
    def addsample(self, x, m, w):
        ...
        if isinstance(self.varyingFeatureGroups, (list,)):
            ...
            if len(groups.shape) == 2:
                ...
                self.synth_data[offset:offset + self.N] = self.dist(features=groups,
    ...
    def sampling_estimate_part(self, j, f, x, X, nsamples=10):
        ...
        for i in range(0, nsamples // 2):
            ...
            sample = self.dist(features=inds[:pos], feature_values=x[0, inds[:pos]], n=1)
            # intervene on z_i
            X_masked[i] = sample
